...
import { createMockCommands, createMockConfig } from "../helpers.ts";
...
Deno.test("resolveRequestDetails - --auth bearer adds Authorization header", () => {
  const config = createMockConfig();
  const commands = createMockCommands({
    _: ["getUsers"],
    auth: "bearer:abc123",
  ...
  assertEquals(request.headers.get("Authorization"), "Bearer abc123");
});

Deno.test("resolveRequestDetails - --auth basic adds base64-encoded Authorization", () => {
  const config = createMockConfig();
  const commands = createMockCommands({
    _: ["getUsers"],
...
Deno.test("resolveRequestDetails - --auth overrides config-defined Authorization", () => {
  const config = createMockConfig({
    requests: {
      getUsers: {
        method: "GET",
        path: "/users",
        headers: { Authorization: "Bearer fromConfig" },
      },
    },
  });
  const commands = createMockCommands({
  ...
  assertEquals(request.headers.get("Authorization"), "Bearer fromCli");
});
