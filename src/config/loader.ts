...
export async function loadConfig(configPath?: string): Promise<Config | null> {
  const defaultPath = join(dirname(fromFileUrl(import.meta.url)), '../../config.yaml');
...
export function resolveRequestDetails(config: Config, commands: ParsedCommands): Request {
  ...
  if (!envDetails) {
    throw new Error(`Environment '${commands.env}' not found in configuration.`);
  }
  ...
  if (commands.data) {
    ...
    if (!dataTemplate) {
      throw new Error(`Data template '${commands.data}' not found in configuration.`);
    }
    ...
    if (paramIndex >= pathParams.length) {
      throw new Error(
        `Not enough path parameters provided for path: '${configuredPath}'. Expected parameter for '${
          ...
          pathParams.length
        } parameter(s) were given. (Request: ${requestName}, Params: [${pathParams.join(', ')}])`,
      );
    }
  ...
  if (paramIndex < pathParams.length) {
    throw new Error(
      `Too many path parameters provided. Path '${configuredPath}' requires ${paramIndex} parameter(s), but ${
        pathParams.length
      } were given. (Request: ${requestName}, Params: [${pathParams.join(', ')}])`,
    );
  }

  const finalPath = processedPath.startsWith('/') ? processedPath.slice(1) : processedPath;
