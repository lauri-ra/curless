import { CurlessError, isCurlessError, getErrorMessage } from '../utils/errors.ts';
  ...
  if (isCurlessError(error)) {
    switch (error.code) {
      ...
      case 'CONFIG_UNREADABLE':
        return `Could not read config file '${getDetail(error, 'configPath')}'. ${error.message}`;
      case 'CONFIG_INVALID_YAML':
        return `Config file '${getDetail(error, 'configPath')}' contains invalid YAML. ${error.message}`;
      case 'SECRETS_FILE_MISSING':
        return `Secrets file not found at '${getDetail(error, 'envFilePath')}'.`;
      case 'SECRETS_FILE_UNREADABLE':
        return `Could not read secrets file '${getDetail(error, 'envFilePath')}'. ${error.message}`;
